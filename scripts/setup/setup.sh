...
LLAMA_API=http://127.0.0.1:${PORT_LLM}/v1
...
install_llama(){ if ! command -v llama-server >/dev/null 2>&1; then tmpdir="$(mktemp -d)"; log "build: llama.cpp from source"; git clone --depth 1 https://github.com/ggerganov/llama.cpp "$tmpdir/llama.cpp"; cmake -S "$tmpdir/llama.cpp" -B "$tmpdir/build" -DLLAMA_BUILD_SERVER=ON; cmake --build "$tmpdir/build" -j"$CPU_CORES" --target llama-server; install -m 755 "$tmpdir/build/bin/llama-server" /usr/local/bin/llama-server; fi; }
...
install_services(){ cat > /etc/systemd/system/cerberus_asist-llama.service <<EOF
[Unit]
...
WorkingDirectory=/opt/cerberus_asist/bot
EnvironmentFile=/opt/cerberus_asist/bot/.env
ExecStart=/opt/cerberus_asist/.venv/bin/python /opt/cerberus_asist/bot/telegram_bot.py
Restart=always
RestartSec=5
NoNewPrivileges=true
PrivateTmp=true
ProtectSystem=full
ProtectHome=true
ReadWritePaths=/opt/cerberus_asist
...
User=cerberus_asist
Group=cerberus_asist
WorkingDirectory=/opt/cerberus_asist/dashboard
ExecStart=/opt/cerberus_asist/.venv/bin/python /opt/cerberus_asist/dashboard/dashboard.py
Restart=always
RestartSec=5
NoNewPrivileges=true
PrivateTmp=true
...
EOF
run systemctl daemon-reload; run systemctl enable cerberus_asist-llama cerberus_asist-bot cerberus_asist-dashboard; }
project_reset(){
...
main(){
  ...
  case "${1:---auto}" in
    ...
    --build|--auto)
      setup_user; prepare_dirs; install_deps; scan; write_state
      ...
      install_python
      install_services
      run systemctl start cerberus_asist-llama cerberus_asist-bot cerberus_asist-dashboard
